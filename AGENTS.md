...
This document is the project rules file for coding agents working in this repository. Every coding session must follow these rules without exception.
...
```text
FORBIDDEN: Any layer           ->  imports another layer of the same module (outside the allowed list in section 1)
FORBIDDEN: Any layer           ->  imports anything from another module directly
FORBIDDEN: Any layer           ->  imports another module's `schemas/public_schemas/*`
FORBIDDEN: api/                ->  imports other module's public/ or api/
FORBIDDEN: public/             ->  imports other module's public/ (no chaining)
FORBIDDEN: services/           ->  imports repositories/, models/, api_schemas, public_schemas, or any other module
FORBIDDEN: services/           ->  imports shared/dependencies/
FORBIDDEN: clients/            ->  imports other module's services/, repositories/, models/, or schemas/
FORBIDDEN: handlers/           ->  imports other module's public/, services/, repositories/, or models/
FORBIDDEN: shared/auth/dependencies.py -> imports any module
FORBIDDEN: Any layer           ->  uses `public_schemas.py`, `dto.py`, or `dtos.py` for inter-module boundaries
FORBIDDEN: users/api           ->  owns system-user, system-role, or system-permission routes
FORBIDDEN: admins/api          ->  owns non-system user registration, profile, or CRUD routes

ALLOWED: services/           ->  shared/auth/jwt.py              (token issuance + password hashing)
ALLOWED: services/           ->  shared/events/bus.py             (event publishing only)
ALLOWED: services/           ->  shared/events/events.py          (event class imports for publishing)
ALLOWED: models/             ->  shared/database/base.py          (owning schema base)
ALLOWED: repositories/       ->  own models/                      (only this layer)
ALLOWED: clients/            ->  own `schemas/public_schemas/responses.py` (boundary validation only)
ALLOWED: public/             ->  own `schemas/public_schemas/requests.py`  (boundary validation only)
ALLOWED: api/                ->  shared/auth/dependencies.py      (get_current_user, require_system_permission)
ALLOWED: api/                ->  shared/dependencies/<module>_deps.py
ALLOWED: main.py (lifespan)  ->  shared/dependencies/<module>_deps.py  (startup wiring only)
ALLOWED: _deps.py            ->  own module's concrete classes + other _deps.py files
```
